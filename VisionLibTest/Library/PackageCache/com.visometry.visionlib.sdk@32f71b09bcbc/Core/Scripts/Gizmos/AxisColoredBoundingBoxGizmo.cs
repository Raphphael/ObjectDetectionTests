        ...
        IEnumerable<BoundingBoxEdge> boundingBoxEdgesInWorldSpace)
    {
        ...
        var edgesToLabel = new[]
        {
            boundingBoxEdgesInWorldSpace
                ...
                .OrderBy(edge => view.camera.WorldToScreenPoint(edge.CenterPoint).y).First(),
            boundingBoxEdgesInWorldSpace
                .Where(edge => edge.axisOrientation == AxisOrientation.Y).OrderBy(
                    edge => view.camera.WorldToScreenPoint(edge.CenterPoint).x).Last(),
            boundingBoxEdgesInWorldSpace
                .Where(edge => edge.axisOrientation == AxisOrientation.Z).OrderBy(
                    edge => view.camera.WorldToScreenPoint(edge.CenterPoint).y).First()
        };
        return edgesToLabel.OrderBy(edge => view.camera.WorldToScreenPoint(edge.CenterPoint).x)
            .ToArray();
    }
