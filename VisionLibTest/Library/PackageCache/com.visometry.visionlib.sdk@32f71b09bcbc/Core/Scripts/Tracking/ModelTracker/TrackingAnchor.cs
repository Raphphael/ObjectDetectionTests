...
namespace Visometry.VisionLib.SDK.Core
{
    ...
    public class TrackingAnchor : MonoBehaviour, ISceneValidationCheck, IParameterHandler
    {
        ...
        [SerializeField]
        [CanBeNull]
        [Tooltip("Unique identifier for this TrackingAnchor within VisionLib. Duplicate names in " +
...
#if UNITY_EDITOR
        [Tooltip("If true, the parameters will be propagated from play mode to edit mode. Otherwise " +
                 "they will be discarded after leaving the play mode.")]
        public bool persistParametersFromPlayMode;
        [Tooltip("If true, the init pose will be propagated from play mode to edit mode. Otherwise " +
                 "they will be discarded after leaving the play mode.")]
        ...
        [SerializeField]
        [Tooltip("Camera that contains the slam tracking result. This camera has to match for all " +
        ...
        public List<RenderedObject> GetRegisteredRenderedObjects()
        {
        ...
        public void UpdateParentAnchor()
        {
            ...
            if (newParent == this.parentAnchor)
            {
                return;
            }
            if (this.parentAnchor != null)
        ...
        private async Task SetWorkSpaceInBackendAsync()
        {
            ...
            ClearNullWorkspaces();
            var workSpaceDefinitions = this.workSpaces.Where(ws => ws.isActiveAndEnabled)
        ...
        public List<SetupIssue> GetGeneralIssues()
        {
            ...
            {
                ISetupIssueSolution solution = Camera.main != null
                    ? new ReversibleAction(
                        () => { this.slamCamera = Camera.main; },
                        this,
                        "Set SLAM camera to Camera.main")
                    : null;
                issues.Add(
                    new SetupIssue(
