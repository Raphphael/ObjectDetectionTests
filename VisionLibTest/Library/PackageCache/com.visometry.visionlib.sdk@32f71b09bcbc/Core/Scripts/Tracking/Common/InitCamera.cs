...
namespace Visometry.VisionLib.SDK.Core
{
    ...
    public class InitCamera : MonoBehaviour
    {
        ...
        private class InitPose
        {
            ...
            public InitPose(Vector4 t, Quaternion r)
            {
                this.type = "visionlib";
                Set(t, r);
            }

            public void Set(Vector4 t, Quaternion r)
            {
                this.t[0] = t.x;
                this.t[1] = t.y;
                this.t[2] = t.z;
        ...
        private static ModelTrackerCommands.InitPose CameraToInitParam(
            Camera camera,
            Matrix4x4 offset)
        {
            Vector4 t;
            Quaternion r;
            CameraHelper.CameraToVLPose(camera, offset, out t, out r);
        ...
        private async Task<WorkerCommands.CommandWarnings> SetInitPoseAsync()
        {
            ...
            ModelTrackerCommands.InitPose initPose = CameraToInitParam(
                this.initCamera,
                this.renderRotationMatrixFromUnityToVL);

            var warnings = await ModelTrackerCommands.SetInitPoseAsync(TrackingManager.Instance.Worker, initPose);
            this.ThrowIfNotAlive();
        ...
        public string GetInitPoseAsString()
        {
            ...
            CameraHelper.CameraToVLPose(cam, this.renderRotationMatrixFromUnityToVL, out t, out r);

            // Convert the transformation into JSON
            InitPose initPose = new InitPose(t, r);
